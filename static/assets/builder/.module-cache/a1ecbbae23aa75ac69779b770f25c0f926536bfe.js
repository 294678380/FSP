...
window.Checkbox = React.createClass({displayName: "Checkbox",
	...
	render:function(){
		return React.createElement("label", null, 
					React.createElement("div", {className: "t-checkbox"}, 
						React.createElement("input", {onChange: this.checkboxChange, name: "t-check", type: this.props.types})
					)
				)
	},
	...
	checkboxChange:function(event){
		if(event.target.checked){
			...
			$(event.target).parents(".t-checkbox").silbings(".t-checkbox").removeClass("active");
		}else{
					...
					tr.map(function(item,key){
						return React.createElement("div", {className: "td", key: key, style: item.style}, 
							...
							item.title == 'checkbox' || item.title == 'radio'?
								React.createElement(Checkbox, {types: item.title})
								:item.title
