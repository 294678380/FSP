...
window.Restful = React.createClass({displayName: "Restful",
	componentDidMount:function(){

	},
	render:function(){
		return React.createElement("div", {ref: "restful_test", className: "Restful restful_test none"}, 
...
window.ContainerBox = React.createClass({displayName: "ContainerBox",
	...
	render:function(){
		return React.createElement("div", {className: "containerbox"}, 
					...
					React.createElement(Restful, null)
				)
	},
	//直接发布
	...
	retest:function(event){
		var target = $(event.target);
		var relname = target.attr("data-relname");
		var restful_test = this.refs["restful_test"];
			console.log(restful_test)
			K.confirm({
				width:"860px",
				title:"测试接口",
				content:restful_test,
				ok:function(){
